    ...
    with open(output_filename, 'w') as csv_file:
        ...
        for target in sorted(results):
            ...
            for finding in findings:
                row.append('Yes') if finding in results[target] else row.append('')
...
def main():
    ...
    parser.add_argument('-i', '--input', required=True, help='Input file containing list of targets (and optional ports)')
    parser.add_argument('filename', help="The output filename")
    parser.add_argument('--format', default='csv', help='The output format to display and save results in, defaults to csv')
    args = parser.parse_args()
    input_filename = args.input
    output_filename = args.filename
    output_format = args.format

    # Read the targets from the input file
    with open(input_filename, 'r') as input_file:
        targets = [line.strip() for line in input_file]
